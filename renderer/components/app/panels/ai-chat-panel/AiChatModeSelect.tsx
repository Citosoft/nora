import type { AiChatModeSelectProps } from "@/components/app/types/aiChatPanelSelects.types";
import { cn } from "@/lib/utils";
import * as SelectPrimitive from "@radix-ui/react-select";
import { Check, ChevronDown, ChevronUp } from "lucide-react";
import { AI_CHAT_PANEL_MODE_MENU } from "@/components/app/logic/aiChatPanelConstants";

export const AiChatModeSelect = ({ value, disabled, onChange }: AiChatModeSelectProps) => {
  const active = AI_CHAT_PANEL_MODE_MENU.find((entry) => entry.value === value) ?? AI_CHAT_PANEL_MODE_MENU[0];
  return (
    <SelectPrimitive.Root
      value={active.value}
      disabled={disabled}
      onValueChange={(next) => {
        if (next === "ask" || next === "agent" || next === "plan") {
          onChange(next);
        }
      }}
    >
      ...
      >
        <span className="max-w-[5rem] truncate">{active.shortLabel}</span>
      ...
      <SelectPrimitive.Portal>
        ...
        >
          ...
          <SelectPrimitive.Viewport className="max-h-[inherit] space-y-0.5 p-1">
            {AI_CHAT_PANEL_MODE_MENU.map((entry) => (
                ...
                <SelectPrimitive.ItemText asChild>
                  <div className="min-w-0 flex-1">
                    <span className="flex items-center gap-1.5 font-medium">
                      ...
                      {entry.disabled ? (
                        <span className="rounded-full border border-border/60 px-1 py-0 text-[9px] leading-none text-muted-foreground">
                          Coming soon
                        </span>
                      ) : null}
                    </span>
